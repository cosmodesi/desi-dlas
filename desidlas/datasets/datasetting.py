...
def split_sightline_into_samples(sightline, REST_RANGE=REST_RANGE, kernel=kernel,v=best_v['all']):
    ...
    fluxes_matrix = np.vstack(list(map(lambda x: x[0][x[1]-kernelrangepx:x[1]+kernelrangepx],
                                   zip(itertools.repeat(flux_padded), np.nonzero(ix_dla_range)[0]+pixel_num_left))))

    lam_matrix = np.vstack(list(map(lambda x: x[0][x[1]-kernelrangepx:x[1]+kernelrangepx],
                                zip(itertools.repeat(lam_padded), np.nonzero(ix_dla_range)[0]+pixel_num_left))))
...
def select_samples_50p_pos_neg(sightline,kernel=kernel):
    ...
    #print(sightline.classification)
