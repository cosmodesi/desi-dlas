...
from desidlas.parameters import best_v
...
class DesiMock: 
    ...
    def read_fits_file(self, spec_path, truth_path, zbest_path):
        ...
        # read spectra data from the fits file above, one can directly get those varibles meanings by their names.
        ...
        if truth_path !=[]:
            ...
            if truth[3].data is None or len(truth[3].data) == 0:
                spec_dlas={}
            else:
        ...
        #extract redshift data from zbest
        if zbest_path!=[]:
            ...
            z_qso = np.zeros(len(spec_id))
            zwarn = np.zeros(len(spec_id))
            spectype = np.zeros(len(spec_id))
            
        if spec_dlas != {}:
            self.data = {spec_id[i]: {'FLUX': flux[i], 'ERROR': error[i], 'z_qso': z_qso[i], 'RA': ra[i], 'DEC': dec[i],
                                      'spectype': spectype[i], 'objtype': objtype[i], 'ZWARN': zwarn[i],
                                      'MASK': pixel_mask[i], 'W1': w1[i], 'W2': w2[i], 'W1_IVAR': w1_ivar[i],
                                      'W2_IVAR': w2_ivar[i], 'DLAS': spec_dlas[spec_id[i]]} for i in
                         range(len(spec_id))}
        else:
            self.data = {spec_id[i]: {'FLUX': flux[i], 'ERROR': error[i], 'z_qso': z_qso[i], 'RA': ra[i], 'DEC': dec[i],
                                      'spectype': spectype[i], 'objtype': objtype[i], 'ZWARN': zwarn[i],
                                      'MASK': pixel_mask[i], 'W1': w1[i], 'W2': w2[i], 'W1_IVAR': w1_ivar[i],
                                      'W2_IVAR': w2_ivar[i], 'DLAS': []} for i in range(len(spec_id))}
    def get_sightline(self, id, camera = 'all', rebin=False, normalize=False):
        ...
        if camera == 'all':
            ...
            blfbound = np.argmin(abs(self.wavelength[:self.split_point_br] - self.wavelength[self.split_point_br]) )
            rrgbound = self.split_point_br + np.argmin(abs(self.wavelength[self.split_point_br:] - self.wavelength[self.split_point_br - 1]))
            rzfbound = np.argmin(abs(self.wavelength[:self.split_point_rz] - self.wavelength[self.split_point_rz]) )
            rzgbound = self.split_point_rz + np.argmin(abs(self.wavelength[self.split_point_rz:] - self.wavelength[self.split_point_rz - 1])) #+1 for mock
            ...
            overlap_flux_1 = np.array(
                [overlap_flux_b[i] if test_1[i] else overlap_flux_r[i] for i in range(-blfbound + self.split_point_br)])
            overlap_error_1 = np.array([overlap_error_b[i] if test_1[i] else overlap_error_r[i] for i in
                                        range(-blfbound + self.split_point_br)])
            overlap_mask_1 = np.array(
                [overlap_mask_b[i] if test_1[i] else overlap_mask_r[i] for i in range(-blfbound + self.split_point_br)])

            test_2 = overlap_error_rr < overlap_error_z
            overlap_flux_2 = np.array([overlap_flux_rr[i] if test_2[i] else overlap_flux_z[i] for i in
                                       range(-rzfbound + self.split_point_rz)])
            overlap_error_2 = np.array([overlap_error_rr[i] if test_2[i] else overlap_error_z[i] for i in
                                        range(-rzfbound + self.split_point_rz)])
            overlap_mask_2 = np.array([overlap_mask_rr[i] if test_2[i] else overlap_mask_z[i] for i in
                                       range(-rzfbound + self.split_point_rz)])

            sightline.flux = np.hstack((
                                       sightline.flux[:blfbound], overlap_flux_1, sightline.flux[rrgbound + 1:rzfbound],
                                       overlap_flux_2, sightline.flux[rzgbound + 1:]))
            sightline.error = np.hstack((sightline.error[:blfbound], overlap_error_1,
                                         sightline.error[rrgbound + 1:rzfbound], overlap_error_2,
                                         sightline.error[rzgbound + 1:]))
            sightline.pixel_mask = np.hstack((sightline.pixel_mask[:blfbound], overlap_mask_1,
                                              sightline.pixel_mask[rrgbound + 1:rzfbound], overlap_mask_2,
                                              sightline.pixel_mask[rzgbound + 1:]))
            sightline.loglam = np.hstack((sightline.loglam[:self.split_point_br],
                                          sightline.loglam[rrgbound + 1:self.split_point_rz],
                                          sightline.loglam[rzgbound + 1:]))
        ...
        #add pixel mask to sightline:
        ind = (sightline.error != float('inf'))&(sightline.pixel_mask == 0)
