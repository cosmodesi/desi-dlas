...
if __name__ == '__main__':
    ...
    for model in ['low','mid'] :

        print(f"Running with model '{model}'")

        with tf.Graph().as_default():
            build_model(hyperparameters,INPUT_SIZE[model],matrix_size[model]) # build the CNN model according to hyperparameters

            with tf.device(''), tf.compat.v1.Session() as sess:
                tf.compat.v1.train.Saver().restore(sess, checkpoint_filename[model]+".ckpt") #load model files

                for sightline in tqdm(r.ravel()):
                    if model == 'low' and sightline.s2n>3 : continue
                    if model == 'mid' and sightline.s2n<3 : continue
                    ...
                    for i in range(0,n_samples,BATCH_SIZE):
                        pred[i:i+BATCH_SIZE], conf[i:i+BATCH_SIZE], offset[i:i+BATCH_SIZE], coldensity[i:i+BATCH_SIZE] = \
                            sess.run([t('prediction'), t('output_classifier'), t('y_nn_offset'), t('y_nn_coldensity')],
                                     feed_dict={t('x'):                 flux[i:i+BATCH_SIZE,:],t('keep_prob'):         1.0}) #get prediction labels
